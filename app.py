...
def add_portfolio_item():
    ...
    try:
        # التحقق مما إذا كان المشروع قد تمت معاينته وتأكيده
        preview_confirmed = request.form.get('preview_confirmed') == '1'
        if preview_confirmed:
            app.logger.info("تمت إضافة مشروع بعد معاينته وتأكيده")
        ...
        app.logger.info(f"Preview confirmed: {preview_confirmed}")
...
@app.route('/admin/portfolio/preview', methods=['POST'])
@login_required
def preview_portfolio_item():
    """عرض معاينة للمشروع قبل نشره"""
    if not current_user.is_admin():
        flash('لا تملك صلاحية الوصول إلى هذه الصفحة', 'danger')
        return redirect(url_for('index'))
    ...
    try:
        ...
        # معالجة الصورة المؤقتة إذا كانت موجودة
        image_url = None
        if 'image' in request.files and request.files['image'].filename:
            image = request.files['image']
            if allowed_file(image.filename):
                image_url = "/static/uploads/temp/preview_" + secure_filename(image.filename)
                # إنشاء المجلد المؤقت إذا لم يكن موجوداً
                temp_dir = os.path.join(app.config['UPLOAD_FOLDER'], 'temp')
                os.makedirs(temp_dir, exist_ok=True)
                # حفظ الصورة مؤقتاً
                image.save(os.path.join(temp_dir, "preview_" + secure_filename(image.filename)))
        
        # استخدام صورة موجودة إذا تم تمريرها (لحالة التعديل)
        ...
        elif video_type == 'local' and 'video_file' in request.files and request.files['video_file'].filename:
            video_file_obj = request.files['video_file']
            if video_file_obj.filename.split('.')[-1].lower() in ['mp4', 'webm', 'ogg']:
                video_filename = "preview_" + secure_filename(video_file_obj.filename)
                temp_dir = os.path.join(app.config['UPLOAD_FOLDER'], 'temp')
                video_path = os.path.join(temp_dir, video_filename)
                video_file_obj.save(video_path)
                video_file = f"/static/uploads/temp/{video_filename}"
        
        # إنشاء مشروع مؤقت للمعاينة
        temp_item = {
            'id': 0,  # معرف مؤقت
            'title': title,
            'title_en': title_en,
            ...
            'category': category,
            'link': link,
            'image_url': image_url or '/static/img/placeholder.jpg',
            'video_type': video_type,
            'video_url': video_url,
            'video_file': video_file,
            'created_at': datetime.now(),
            'carousel_images': [],  # لا داعي للكاروسيل في المعاينة
            'comments': [],  # لا تعليقات في المعاينة
            'views_count': 0,
            'likes_count': 0
        }
        
        # إضافة بيانات الجلسة للعودة إلى الصفحة السابقة
        session['prev_page'] = request.form.get('prev_page', 'add')  # 'add' أو 'edit'
        if 'portfolio_id' in request.form:
            session['edit_portfolio_id'] = request.form.get('portfolio_id')
        
        return render_template('admin/portfolio_preview.html', 
                               item=temp_item, 
                               is_preview=True)
    
    except Exception as e:
        app.logger.error(f"خطأ في معاينة المشروع: {str(e)}")
        flash(f"حدث خطأ أثناء معاينة المشروع: {str(e)}", "danger")
        return redirect(url_for('admin_portfolio_management'))
...
def update_portfolio_item(portfolio_id):
    ...
    if request.method == 'POST':
        ...
        preview_confirmed = request.form.get('preview_confirmed') == '1'
        if preview_confirmed:
            app.logger.info(f"تم تحديث مشروع (ID: {portfolio_id}) بعد معاينته وتأكيده")
